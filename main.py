...
BOT_TOKEN = "7793802205:AAFluAbe_J_63NHhI2ziZF_lugdASe60txQ"
PAYMENT_CHANNEL = "@proof_payment_mishbah" #add payment channel here including the '@' sign
OWNER_ID = 1905864597 #write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@payments_hbot"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
...
def start(message):
   try:
    ...
    if msg == '/start':
        ...
        channel_button = telebot.types.InlineKeyboardButton(
               text="Join Our Channel", url=f"https://t.me/{CHANNELS[0][1:]}")
        markup.add(channel_button)
        channel_button = telebot.types.InlineKeyboardButton(
               text="Join Our Channel", url=f"https://t.me/+zq5siXMLSVFmMzc1")
        markup.add(channel_button)
...
def send_message_to_all_users(message):
    try:
        ...
        if message.chat.id == OWNER_ID:
            ...
            msg_text = message.text[13:].strip()  # Remove "/send_message " from the text
            if not msg_text:
                bot.send_message(message.chat.id, "❌ Please provide a message to send to all users.")
                return

            # Load the list of users from the JSON data
            try:
                with open('users.json', 'r') as file:
                    data = json.load(file)
            except FileNotFoundError:
                bot.send_message(message.chat.id, "❌ The user data file (users.json) is missing.")
                return

            # Get all user IDs from the 'referred' dictionary
            all_users = list(data.get('referred', {}).keys())

            if not all_users:
                bot.send_message(message.chat.id, "❌ No users found to send the message to.")
                return
            ...
            for id in all_users:
                try:
                    ...
                    failure_count += 1
                    print(f"Could not send message to {id}: {str(e)}")

            # Send confirmation to the admin
            bot.send_message(message.chat.id, f"✅ Message sent to {success_count} users. {failure_count} failed.")
        else:
            bot.send_message(message.chat.id, "❌ You are not authorized to use this command.")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ An error occurred while sending the message. Please try again later. Error: {str(e)}")
        bot.send_message(OWNER_ID, f"⚠️ An error occurred while sending a message to all users. Error: {str(e)}")
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Set Wallet":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Send your fam Wallet upi.\n\n note :- this is only for fam users or upi id lenghth only 14_",
                                parse_mode="Markdown", reply_markup=keyboard)
        # Next message will call the name_handler function
        bot.register_next_step_handler(message, UPI)
...
def UPI(message):
   try:
    ...
    if len(message.text) == 14:
        ...
        bot.send_message(message.chat.id, "*💹Your fam wallet set to " +
                         data['wallet'][user]+"*", parse_mode="Markdown")
        json.dump(data, open('users.json', 'w'))
        return menu(message.chat.id)
    else:
        bot.send_message(
            message.chat.id, "*⚠️ It's Not a Valid fampay upi!*", parse_mode="Markdown")
        return menu(message.chat.id)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
